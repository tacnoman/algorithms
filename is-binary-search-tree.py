...
def is_bst(node, lower_lim=None, upper_lim=None):
    ...
    if upper_lim != None and upper_lim < node.value:
        return False
...
def test_bst(head, should_be):
    if is_bst(head) == should_be:
        print('.')
        return

    error_message = "Error. The current value is %s but expected %s" % (str(not should_be), str(should_be))
    print(error_message)
    print(head)


test_bst(head0, False)
test_bst(head1, False)
test_bst(head2, False)
test_bst(head3, True)
test_bst(head4, False)
